...
# loop through Compact calls datasets and read them in
...
definitions <- make_lookup(
  output_path = base_mods$path[1]
  )

data_merge <- dplyr::bind_rows(cc_df, nocc_df)

# no compact call
nocc_df <- process_output(
  file_df = base_mods,
  date_df = qm_convert,
  verbose = TRUE
  ) %>%
  dplyr::filter(
    year >= year_subs$start[1],
    year <= year_subs$end[y]
  ) %>%
  dplyr::mutate(
    compact_call = comp_mods$compact_call[y],
...
output_folder <- paste0(
  base_model_ID, "-", base_climate, "_",  base_model_ID_suffix,
  " vs. ",
  compare_model_ID,  "-", compare_climate, "_",  compare_model_ID_suffix)
...
currwd <- "G:/.shortcut-targets-by-id/1B8Jfl31Nww6VN-dRe1H6jov-ogkJhMBW/2022 Modeling/Model Runs/Compact Call Analysis/"
...
for (i in 1:temp_run_number){
  ...
  if(i == 1){
    ...
    data <- fread(paste0(currwd,  compare_model_ID, "/", file_prefix[i], ".OutputSheet.csv"),
                  header = FALSE)
    ...
    qm_convert <- read_csv("D:/cob/compact_call/qm_to_date_conversion2.csv")
    #qm_convert
