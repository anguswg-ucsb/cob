...
get_model_comp <- function(
  ...
  suppressWarnings({
    ...
    #   )
    # )
    # paste0(base_out$model_num, "_", gsub(paste0("_", base_out$model_num), "", base_out$extra_info))
    # paste0(comp_out$model_num, "_", gsub(paste0("_", comp_out$model_num), "", comp_out$extra_info))
    scenario_name <- c(
      paste0(base_out$model_num, "_", gsub(paste0("_", base_out$model_num), "", base_out$extra_info)),
      paste0(comp_out$model_num, "_", gsub(paste0("_", comp_out$model_num), "", comp_out$extra_info))
    )
    ...
    if (!dir.exists(output_dir)) {
      ...
      if(!dir.exists(paste0(output_dir, "/", output_folder))) {
        ...
        stop(paste0("Model comparison folder already exists:\n\n  ", output_dir, "/", output_folder))
    ...
    drought_index <-
      outputs %>%
      process_drought_index(drought_qm = drought_qm) %>%
      dplyr::mutate(
    ...
    # remove Drought Watch
    drought_plot_lst <- drought_plot_lst[!grepl("Drought Watch", drought_plot_lst)]
    ...
    # # make drought  GT table
    # drought_tgrob <-
    #   drought_index %>%
    #   make_drought_index_tgrob(mod_runs = scenario_name)
    #
    drought_tgrob <-
      outputs %>%
      process_drought_index_wide(drought_qm = drought_qm) %>%
      make_drought_index_tables2(
    ...
    # save 1AB plot
    ggplot2::ggsave(
      filename = paste0(model_comp_dir, "/", "1ab. May 1 Annual Reservoir Contents - Time Series Plot 3x1.png"),
      width  = 16,
      height = 8,
      gridExtra::grid.arrange(
        drought_plots[["Barker Reservoir Contents"]],
        ...
        drought_plots[["Total Upper Reservoir Contents"]],
        nrow   = 3,
        top    = "1ab. May 1 Annual Reservoir Contents - Time Series Plot",
        right  = "",
        bottom = ""
      )
    )
    ...
    # drought_tbl <-
    #   drought_index %>%
    #   make_drought_index_tables(mod_runs = scenario_name)

    # save table 1ac
    gt::gtsave(
      data     = drought_tbl,
      filename = paste0(model_comp_dir, "/", "1ac. Drought Reliability table.png"),
      zoom     = 0.9
    )
    ...
    cbt_quota_tbl  <-
      cbt_quota %>%
      process_cbt_quota_tbl(borrow = borrow) %>%
      make_cbt_quota_tbl2()
    ...
    all_cob_res <-
      outputs %>%
      process_all_cob_res(definitions_df = definitions) %>%
      dplyr::mutate(
        model_run = factor(model_run, levels = c(scenario_name))
      )

    # unique sites to plot over
    all_cob_sites <- unique(all_cob_res$name)

    # plot 2L, loop through each site and plot
    all_cob_res_lst <- lapply(1:length(all_cob_sites), function(i) {

      extract_df <-
        all_cob_res %>%
        dplyr::filter(name == all_cob_sites[i])

      message(paste0("Plotting: All COB Reservoirs - ", all_cob_sites[i]))

      all_cob_res_plot <-
        make_all_cob_res_plot(
          df         = extract_df,
      ...
      stats::setNames(c(all_cob_sites))

    # save plot 2L
    ggplot2::ggsave(
      filename = paste0(model_comp_dir, "/", "2m. City of Boulder Reservoirs - Quarter-Monthly Time Series Plot 4x1.png"),
      width    = 14,
      height   = 8,
      gridExtra::grid.arrange(
        all_cob_res_lst[["Reservoir_3_Content"]],
        all_cob_res_lst[["Reservoir_1_Content"]],
        all_cob_res_lst[["Total_Upper_Storage"]],
        all_cob_res_lst[["COB_Total_Boulder_Res_Storage"]],
        nrow   = 4,
        top    = "2m. City of Boulder Reservoirs - Quarter-Monthly Time Series Plot",
        right  = ""
      )
    )
